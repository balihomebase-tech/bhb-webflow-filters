(function(){
  ...
  var CFG={
    GRID_ID:'rentals-wrapper',CARD_SEL:'.w-dyn-item',STEP:9
  };
  ...
  var LOC_COORDS={
    'canggu':[115.13650,-8.65062],'pererenan':[115.12346,-8.64904],'seseh':[115.11505,-8.64560],'cemagi':[115.11502,-8.62971],'kaba kaba':[115.13919,-8.59345],'kaba-kaba':[115.13919,-8.59345],'cepaka':[115.14526,-8.59917],'tumbak bayuh':[115.14562,-8.61484],'buwit':[115.12362,-8.59905],'dalung':[115.17258,-8.61147],'bingin':[115.09200,-8.81200],'uluwatu':[115.08800,-8.82828],'uluwatu center':[115.08800,-8.82828],'ungasan':[115.16562,-8.82695],'ubud':[115.26229,-8.50690],'ubud center':[115.26229,-8.50690],'kedungu':[115.09045,-8.60254],'nyanyi':[115.11025,-8.61237],'pandak gede':[115.12800,-8.58500],'nyambu':[115.13200,-8.57800],'tanah lot':[115.12604,-8.58208]
  };
  var CHIP_PRESETS={
    IDR:[{
  ...
  var state={
    availability:'Any',bedrooms:[],locations:[],currency:'IDR',priceMin:null,priceMax:null,keyword:''
  };
  var slider={
    base:{
      min:0,max:5000000
  ...
  function norm(v){
    return String(v||'').toLowerCase().trim().replace(/[-\u2013\u2014]/g,' ').replace(/[^\w\s]/g,'').replace(/\s+/g,' ');
  }
  ...
  function convertAmount(amount,from,to){
    ...
    if(window.debugCurrency&&typeof window.debugCurrency.convertAmount==='function')return window.debugCurrency.convertAmount(amount,from,to);
    return amount;
  }
  ...
  function cacheEls(){
    ...
    i++){
      ...
      for(var j=0;
      j<opts.length;
      j++)vals.push(opts[j].dataset.value||'');
    ...
    i++){
      (function(opt){
        opt.addEventListener('click',function(e){
          ...
          for(var k=0;
          k<opts.length;
          k++)opts[k].classList.remove('is-active');
    ...
    i++){
      (function(opt){
        opt.addEventListener('click',function(e){
          ...
          k<opts.length;
          k++){
            if(opts[k].classList.contains('is-active')&&opts[k].dataset.value!=='Any')selected.push(opts[k].dataset.value);
          }
  ...
  function clearAll(e){
    ...
    function resetToAny(field,trigText,label){
      ...
      for(var i=0;
      i<opts.length;
      i++)opts[i].classList.remove('is-active');
      var anyOpt=field.querySelector('.filter-option[data-value="Any"]');
  ...
  function bindEvents(){
    ...
    if(el.locTrigger){
      el.locTrigger.addEventListener('click',function(e){
        ...
        else if(locDropOpen&&map)setTimeout(function(){
          map.resize();
        },
        80);
      })
    ...
    if(el.locDropdown)el.locDropdown.addEventListener('click',function(e){
      e.stopPropagation();
    })
    ;
  ...
  function getData(card){
    ...
    return{
      name:(inner.dataset.name||'').toLowerCase(),code:(inner.dataset.code||'').toLowerCase(),locRaw:inner.dataset.location||'',loc:norm(inner.dataset.location||''),rooms:parseInt(inner.dataset.rooms||'0',10),price:parseFloat(inner.dataset.price||'0'),currency:(inner.dataset.currency||'').toUpperCase(),avail:inner.dataset.availableDate?'Rented':'Available'
    };
  }
  ...
  function computeBaseBounds(){
    ...
    if(isFinite(lo)&&isFinite(hi))slider.base={
      min:lo,max:hi
    };
  }
  function fixSliderDOM(){
    ...
    if(embed){
      ...
      for(var i=0;
      i<inputs.length;
      i++)inputs[i].style.pointerEvents='auto';
    }
  ...
  function updateSliderForCurrency(currency){
    ...
    if(minT)minT.value=Number(minV).toLocaleString('en-US',{
      maximumFractionDigits:0
    })
    ;
    if(maxT)maxT.value=Number(maxV).toLocaleString('en-US',{
      maximumFractionDigits:0
    })
    ;
    state.priceMin=minV;
  ...
  function initPricePanel(){
    ...
    function bindTextInput(inputEl,which){
      ...
      inputEl.addEventListener('change',function(){
        ...
        if(which==='min'){
          slider.minRatio=valToRatio(Math.min(v,ratioToVal(slider.maxRatio)));
        }
        else{
          slider.maxRatio=valToRatio(Math.max(v,ratioToVal(slider.minRatio)));
        }
        fullRender();
        applyFilters();
      })
    ...
    slider.active={
      min:slider.base.min,max:slider.base.max
    };
  ...
  function buildAreas(){
    ...
    a++){
      ...
      ;
      if(children.length)areas.push({
        id:rule.id,label:rule.label,children:children
      })
      ;
    }
    ...
    ;
    if(other.length)areas.push({
      id:'other-area',label:'Other area',children:other
    })
    ;
  ...
  function mountLocUI(){
    ...
    locUI={
      searchInput:panel.querySelector('.location-search-input'),treeScroll:panel.querySelector('.tree-scroll'),pillScroll:panel.querySelector('.pill-scroll'),selectedInfo:panel.querySelector('#locSelectedInfo'),btnClear:panel.querySelector('.loc-btn-clear-inline'),btnApply:panel.querySelector('.loc-btn-apply-inline')
    };
  ...
  function renderLocLists(){
    ...
    a++){
      ...
      var isOpen=openAreas[area.label]||draftLocs.some(function(l){
  ...
  function openLocDrop(force){
    ...
    if(locDropOpen&&map)setTimeout(function(){
      map.resize();
    },
    80);
  ...
  function loadMapSDK(cb){
    ...
    if(ex)return ex.addEventListener('load',cb,{
      once:true
    })
    ;
  ...
  function initMap(){
    ...
    map=new maptilersdk.Map({
      container:'bhbMap',style:MAP_STYLE,center:[115.1889,-8.4095],zoom:9.3
    })
    ;
  ...
  function syncMapWith(locations){
    ...
    if(!pts.length){
      map.flyTo({
        center:[115.1889,-8.4095],zoom:9.3,duration:450
      })
      ;
      return;
    }
    ...
    i<pts.length;
    i++){
      var label=(labelByNorm[pts[i].loc]||pts[i].loc).replace(/\b\w/g,function(c){
        return c.toUpperCase();
      })
      ;
      markers.push(new maptilersdk.Marker({
        element:makePin(label),anchor:'bottom'
      })
      .setLngLat(pts[i].p).addTo(map));
    }
    if(pts.length===1){
      map.flyTo({
        center:pts[0].p,zoom:12.2,duration:450
      })
      ;
      return;
    }
    ...
    for(var i=0;
    i<pts.length;
    i++)bounds.extend(pts[i].p);
    map.fitBounds(bounds,{
      padding:{
        top:40,right:40,bottom:40,left:40
      },
      maxZoom:12,duration:450
    })
    ;
  }
  ...
  function init(){
    ...
    for(var i=0;
    i<allDrops.length;
    i++)allDrops[i].style.display='none';
